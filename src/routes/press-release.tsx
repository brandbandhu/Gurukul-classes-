      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {PRESS.map((item) => (
              <article key={item.title} className="group flex flex-col md:flex-row gap-6 rounded-3xl border border-border bg-white p-6 md:p-8 shadow-soft hover:shadow-glow transition-all">
                <div className="md:w-32 flex md:flex-col items-center md:items-start gap-3 md:gap-1">
                  ...
                  <span className="text-xs font-bold tracking-widest text-brand-red uppercase">{item.date}</span>
                </div>
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-brand-ink group-hover:text-brand-red transition-colors">{item.title}</h3>
